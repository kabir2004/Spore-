/**
 * Zustand store for workspace blocks and metadata.
 * Persisted to localStorage (key: spore-workspace). Hydrated from Supabase on load via useWorkspaceSync.
 * Mock initial state used when no workspace is loaded; replaced by hydrateWorkspace when sync runs.
 */
